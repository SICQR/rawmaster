#!/usr/bin/env python3
"""
RAWMASTER Ñ Smash Daddys Audio Tools
...
def check_license():
    ...
    try:
        ...
        if license_file.exists():
            print("  ??  Offline Ñ using cached license\n")
        else:
            print("  ? Cannot validate license Ñ check your internet connection")
            sys.exit(1)
...
def remaster(audio_path: Path, output_dir: Path) -> Path:
    print(f"  ?  Remastering {audio_path.name}É")
...
def extract_midi(stem_paths: dict, output_dir: Path, midi_all: bool = False):
    ...
    for stem_name in targets:
        ...
        print(f"  ? Extracting MIDI from {stem_name}É")
        warn_if_first_run()
...
def detect_info(audio_path: Path, output_dir: Path) -> dict:
    print(f"  ? Detecting BPM and keyÉ")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="rawmaster",
        description="Smash Daddys Audio Tools Ñ stem separation, remaster, MIDI, BPM/key",
    )
